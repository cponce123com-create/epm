...
const DEFAULT_CATEGORIES = [
  ...
  { name: "Políticos", slug: "politicos", color: "#6C3483", description: "Perfiles y seguimiento de figuras políticas locales" },
];
...
export async function seed() {
  try {
    ...
    // ── Seed sample articles (only if admin email is configured) ─────
    if (adminEmail) {
      ...
      if (existingArticles.length === 0) {
        ...
        const [denunciaCategory] = await db
          .select({ id: categoriesTable.id })
          .from(categoriesTable)
          .where(eq(categoriesTable.slug, "denuncia"));

        const [opinionCategory] = await db
          .select({ id: categoriesTable.id })
          .from(categoriesTable)
          .where(eq(categoriesTable.slug, "opinion"));

        const [ciudadCategory] = await db
          .select({ id: categoriesTable.id })
          .from(categoriesTable)
          .where(eq(categoriesTable.slug, "ciudad"));
